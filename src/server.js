...
server.use("/jobnow/curriculum/", express.static("src/modules/candidatos/curriculum/"));
server.use(cors());
server.use(bodyParser.urlencoded({extended: false}));
server.use(bodyParser.json());
server.use("/jobnow", routes);
...
const config = {
  ...
  databaseURI: databaseUri || 'mongodb://localhost:27017/job_now',
...
const dashboard = new ParseDashboard({
  ...
  "users" : [
    {
      "user" : process.env.DASHBOARD_USERNAME || "NeltonMenata",
      "pass" : process.env.DASHBOARD_PASSWORD || "nelton123"
    }
  ],
  "trustProxy": 1
});
